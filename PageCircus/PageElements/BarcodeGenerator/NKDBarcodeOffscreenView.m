...
- (void)drawRect:(NSRect)rect
{
   ...
   // NSMutableParagraphStyle	*style = [[NSMutableParagraphStyle alloc] init];
        ...
        if ([codeString characterAtIndex:i] == '1')
        {
            ...
            if (i == [codeString length]-1)
            {
                path = [NSBezierPath bezierPathWithRect:NSMakeRect(curPos,
                                                                   ...
                                                                   [barcode barWidth] * barCount,
                                                                   [barcode barTop:lastBarIndex] - [barcode barBottom:lastBarIndex] )];
                [[NSColor colorByName:@"black"] set];
                [path setLineWidth:0.0];
                [path fill];
                [[NSColor colorByName:@"white"] set];
            }
        }
        else
        {
    ...
    if ([barcode printsCaption])
    {
        ...
        NSFont *font = [NSFont fontWithName: @"OCR-B-10 BT" size: [barcode fontSize]];
		if (font == nil)
            font = [NSFont userFixedPitchFontOfSize: [barcode fontSize]];
		...
		if ((nil != leftCaption) && (NO == [leftCaption isEqualToString:@""])) {
			kerning = 0.0;
			sAttr = [[NSMutableAttributedString allocWithZone:[self zone]] initWithString:leftCaption attributes:[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorByName:@"black"], NSForegroundColorAttributeName,font,NSFontAttributeName,leftAligmentStyle,NSParagraphStyleAttributeName,[NSNumber numberWithFloat:kerning],NSKernAttributeName,nil]];
		...
		if ((nil != caption) && (NO == [caption isEqualToString:@""])) {
			...
			if (((nil != leftCaption) && (NO == [leftCaption isEqualToString:@""])) || (1 < [captions count])) {
				...
				for (cCount = 0; cCount < [captions count]; cCount++) {
					kerning = 0.0;
					sAttr = [[NSMutableAttributedString allocWithZone:[self zone]] initWithString:[captions objectAtIndex:cCount] attributes:[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorByName:@"black"], NSForegroundColorAttributeName, font,NSFontAttributeName,leftAligmentStyle,NSParagraphStyleAttributeName,[NSNumber numberWithFloat:kerning],NSKernAttributeName,nil]];
					captionSize = [sAttr size];
				...
				kerning = (container - captionSize.width) / (float)(2 * captionLength);
				range = NSMakeRange(0,captionLength);
				[sAttr setAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorByName:@"black"], NSForegroundColorAttributeName, font, NSFontAttributeName,leftAligmentStyle,NSParagraphStyleAttributeName,[NSNumber numberWithFloat:2.0 * kerning],NSKernAttributeName,nil] range:range];
		...
		if ((nil != rightCaption) && (NO == [rightCaption isEqualToString:@""])) {
			kerning = 0.0;
			sAttr = [[NSMutableAttributedString allocWithZone:[self zone]] initWithString:rightCaption attributes:[NSDictionary dictionaryWithObjectsAndKeys:[NSColor colorByName:@"black"], NSForegroundColorAttributeName,font,NSFontAttributeName,leftAligmentStyle,NSParagraphStyleAttributeName,[NSNumber numberWithFloat:kerning],NSKernAttributeName,nil]];
